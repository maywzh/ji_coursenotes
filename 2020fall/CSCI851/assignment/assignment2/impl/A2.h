...
class Electorate
{
	...
	public:
		...
		void assignCandidate(Party *p,int,int sizeParty,int sizeIssue);	//This is going to assign the candidates from the party to Electorate
...
void generateElectorate(Electorate *electorateArray,int sizeElectorate,Stance *s,int sizeIssue,Party *parties,int sizeParty);
...
void localDebate(Electorate *electorateArray,Stance *issueList,int size,int sizeParty,int index);		//This effects the popularity of candidate and stance of electorate.
void candidateEventDonationElectorate(Electorate *electorateArray,int sizeParty,int index);								//This effect the popularity of candidate
void candidateEventPopularity(Electorate *electorateArray,int sizeParty,int index);		
void localIssueEvent(Electorate *electorateArray,Stance *issueList,int size,int index);	//This effect the stance of each electorate 
...
void leadersDebate(Electorate *electorateArray,int,Stance *issueList,int size,Party* parties,int sizeParty);		  //This effects the popularity of leader and stance of electorate.
void printAllLeadersPopularity(Party *parties,int partySize);
void leaderEventPopularity(Electorate *electorateArray,int,Stance *issueList,int sizeIssue,Party* parties,int sizeParty);		  //This effect the popularity of leader
void updateLeaderPopularityBaseOnYear(Party *parties,int partySize);
void leaderEventExperinceImpact(Electorate *electorateArray,int,Stance *issueList,int sizeIssue,Party* parties,int sizeParty);	
void nationalIssueEvent1(Electorate *electorateArray,int,Stance *issueList); //This effect the stance of all electorate 
//CAMPAIGN DAY
void distributeFund(Party *parties,int fundRecived,int sizeParty);											   //distributing the fund that we got. 
void  dailySpending(Electorate *electorateArray, Party *parties,int sizeIssue,int numElectorate);							//This is the effect of the fund
void campaignDay(int,int numElectorate,Electorate *electorateArray, Party *parties,Stance *issueList,int sizeIssue, int sizeParty);
void updateElectorateStance(Electorate *electorateArray,int,Stance *issueList,int size);
...
void processVotes(Electorate *Electorates,int,int,Party* parties);			//using distance formula, electorate, vote to candidate with less distance 
//Result
void result(Electorate *Electorates,int sizeParty,Party* parties);				//announcing the result
