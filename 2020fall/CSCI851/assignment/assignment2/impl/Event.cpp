...
//This Function update the opinion of all Electorate.
void updateElectorateStance(Electorate *electorateArray,int electorateSize,Stance *issueList,int size)
{
...
//LOCAL EVENT 
//This Event is going to effect the popularity of each CANDIDATE in the Electorate and opinion of EACH Electorate.
//It is going to update the stance and popularity.
void localDebate(Electorate *electorate,Stance *issueList,int issueSize,int sizeParty,int index)
{
	cout<<"*Local Debate for Electorate "<<index+1<<endl;
...
void candidateEventDonationElectorate(Electorate *electorate,int sizeParty,int index)
{
	cout<<"*Canidate Donation Event for Electorate "<<index+1<<endl;
...
void candidateEventPopularity(Electorate *electorate,int sizeParty,int index)
{
	cout<<"*Canidate Popularity Event for Electorate "<<index+1<<endl;
	cout<<"Candidate Popularity before the event:\n";
	electorate[index].printCandidatePopularity(sizeParty);
	cout<<"\nCandidate Popularity after the event:\n";
...
void localIssueEvent(Electorate *electorateArray,Stance *issueList,int size,int index)
{
	uniform_int_distribution<unsigned> RandomIssue(0,4);
	int issueIndex=RandomIssue(randEng3);
	cout<<"*Canidate Issue("<<issueList[issueIndex].getIssue()<<") Event for Electorate "<<index+1<<endl;
	cout<<"Stance before the Event: ";
...
void leadersDebate(Electorate *electorateArray,int electorateSize,Stance *issueList,int sizeIssue,Party* parties,int sizeParty)
{
...
void leaderEventPopularity(Electorate *electorateArray,int electorateSize,Stance *issueList,int sizeIssue,Party* parties,int sizeParty)
{
...
void leaderEventExperinceImpact(Electorate *electorateArray,int electorateSize,Stance *issueList,int sizeIssue,Party* parties,int sizeParty)
{
...
void nationalIssueEvent1(Electorate *electorateArray,int electorateSize,Stance *issueList,int sizeIssue)
{
	uniform_int_distribution<unsigned> RandomIssue(0,4);
	int issueIndex=RandomIssue(randEng3);
...
void  dailySpending(Electorate *electorateArray, Party *parties,int partySize,int numElectorate)
{
	cout<<"*Daily Spending by Campaign Manager*"<<endl;
	cout<<"Electorate\tCampaignManagerParty\tFundImpact\tCurrentPopularity\tPreviousFund\tCurrentPopularity\tCurrentFund"<<endl;
	for(int i=0;i<numElectorate;i++)
	{
		electorateArray[i].useFund(parties,partySize,i,numElectorate);
	}
}
void campaignDay(int numOfDay,int electorateSize,Electorate *electorateArray, Party *parties,Stance *issueList,int sizeIssue, int sizeParty)
{
	for(int i=0;i<numOfDay;i++)
	{
		...
		//The reason that it is not inside the the Electorate loop is because, National event effect all the electorate not each of them.
...
void processVotes(Electorate *Electorates,int eSize,int issueSize,Party* parties)
{
	cout<<"*************************Election Day*************************"<<endl;
	cout<<"Electorate\tParty1\tParty2\tParty3\tWinner"<<endl;
...
void result(Electorate *Electorates,int sizeElectorate,Party* parties)
{
	cout<<"*************************Result*************************"<<endl;
