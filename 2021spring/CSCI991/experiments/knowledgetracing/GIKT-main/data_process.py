...
def data_process(args):
    ...
    train_data_directory = os.path.join(
        args.data_dir, args.dataset, args.dataset + '_train.csv')
    valid_data_directory = os.path.join(
        args.data_dir, args.dataset, args.dataset + '_test.csv')
    ...
    args.test_seqs, test_student_num, test_max_skill_id, test_max_question_id, _ = load_data(
        test_data_directory, args.field_size, args.max_step)
    args.valid_seqs, _, _, _, _ = load_data(
        valid_data_directory, args.field_size, args.max_step)
    ...
    matrix_directory = os.path.join(
        args.data_dir, args.dataset, args.dataset + '_skill_matrix.txt')
    args.skill_matrix = np.loadtxt(matrix_directory)  # multi-skill

    # [[neighbor skill/question] for all qs]]
    qs_adj_list, interactions = build_adj_list(
        args.train_seqs, args.test_seqs, args.skill_matrix, args.qs_num)
    args.question_neighbors, args.skill_neighbors = extract_qs_relations(
        qs_adj_list, args.skill_num, args.qs_num, args.question_neighbor_num, args.skill_neighbor_num)
...
def build_adj_list(train_seqs, test_seqs, skill_matrix, qs_num):
    ...
    for seqs in [train_seqs, test_seqs]:
        for seq in seqs:
            ...
            for step in seq:
                adj_list[step[1]] = np.reshape(np.argwhere(
                    skill_matrix[step[0]] == 1), [-1]).tolist()
                adj_num[step[1]] += 1
...
def extract_qs_relations(qs_list, s_num, qs_num, q_neighbor_size, s_neighbor_size):
    # the first s_num rows are 0
    question_neighbors = np.zeros([qs_num, q_neighbor_size], dtype=np.int32)
            ...
            if i == 1:
                ...
                for k in range(n_split):
                    ...
                    for i in range(len(feature_answer_list)):
                        split_list.append(
                            feature_answer_list[i][k * max_seq_len:end_index])
...
def sample_hist_neighbors(seqs_size, max_step, hist_num, skill_index):
    ...
    for i in range(seqs_size):
        ...
        for j in range(1, max_step):
            same_skill_index = [k for k in range(
                j) if seq_skill_index[k] == seq_skill_index[j]]

            if hist_num != 0:
                # [0,j] select M
                if len(same_skill_index) >= hist_num:
                    seq_hist_index.append(np.random.choice(
                        same_skill_index, hist_num, replace=False))
                else:
                    if len(same_skill_index) != 0:
                        seq_hist_index.append(np.random.choice(
                            same_skill_index, hist_num, replace=True))
                    else:
                        seq_hist_index.append(
                            ([max_step-1 for _ in range(hist_num)]))
            else:
                seq_hist_index.append([])
...
def format_data(seqs, max_step, feature_size, hist_num):
    ...
    features_answer_index = pad_sequences(
        seqs, maxlen=max_step, padding='post', value=0)
    target_answers = pad_sequences(np.array(
        [[j[-1] - feature_size for j in i[1:]] for i in seqs]), maxlen=max_step-1, padding='post', value=0)
    skills_index = features_answer_index[:, :, 0]
    hist_neighbor_index = sample_hist_neighbors(
        len(seqs), max_step, hist_num, skills_index)  # [batch_size,max_step,M]
...
class DataGenerator(object):
    ...
    def next_batch(self):
        ...
        batch_seqs = self.seqs[self.batch_i *
                               self.batch_size:(self.batch_i+1)*self.batch_size]
        self.batch_i += 1
        ...
        format_data_list = format_data(
            batch_seqs, self.max_step, self.feature_size, self.hist_num)
