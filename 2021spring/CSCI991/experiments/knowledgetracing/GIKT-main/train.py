        ...
        if train_dkt:
            ...
            for epoch in tqdm(range(args.num_epochs)):
                ...
                while not train_generator.end:
                    ...
                    binary_pred, pred, loss = model.train(
                        sess, features_answer_index, target_answers, seq_lens, hist_neighbor_index)
                ...
                print("\ntrain loss = {0},auc={1}, accuracy={2}".format(
                    train_loss, auc_value, accuracy))
                write_log(args, model_dir, auc_value,
                          accuracy, epoch, name='train_')
                ...
                while not valid_generator.end:
                    ...
                    binary_pred, pred = model.evaluate(
                        sess, features_answer_index, target_answers, seq_lens, hist_neighbor_index, valid_step)
                    #overall_loss += loss
                    for seq_idx, seq_len in enumerate(seq_lens):
                        preds.append(pred[seq_idx, 0:seq_len])
                        binary_preds.append(binary_pred[seq_idx, 0:seq_len])
