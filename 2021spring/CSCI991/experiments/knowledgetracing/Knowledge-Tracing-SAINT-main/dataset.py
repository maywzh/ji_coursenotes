...
class DKTDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        input = {"input_ids": input_ids, "input_rtime": input_rtime.astype(
            np.int), "input_cat": input_cat}
...
def get_dataloaders():
    ...
    train_dataset = DKTDataset(
        train.values, n_skills=n_skills, max_seq=config.MAX_SEQ)
    val_dataset = DKTDataset(
        val.values, n_skills=n_skills, max_seq=config.MAX_SEQ)
