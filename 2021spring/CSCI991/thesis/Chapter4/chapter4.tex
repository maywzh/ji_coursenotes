...
\begin{enumerate}
  \item Matching: The purpose of this part is to generate a series of candidate recommendation exercises. In this paper, a multi-path recall strategy is applied, that is, multiple recall strategies are applied to form corresponding sub-candidate sets and then merge them into a candidate set by merging method. Three matching strategies based on collaborative filtering, statistical information and user preferences are respectively applied in this paper, and the weighted average method is used for ranking and merging.
...
After multi-channel recall merging settlement, a candidate set of exercises \(\mathbf{E}_{candidate}\) is generated, each of which contains a corresponding weight and source to calculate the recommended loss function. Since the purpose of recall is to provide candidate recommendations for the final ranking stage, a final recommendation weight will be output for each exercise in the ranking stage for the construction of the final recommendation list.
