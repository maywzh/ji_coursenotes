...
%该数据集的习题知识点数目分布如图所示。不同的知识点所涉及的题目数量不同，我们定义知识点\(j\)涉及的题目集合为\(\mathbb{E}_j\)。记知识点标签出现次数阈值\(\tau^j \),则可以将习题的知识点按照出现的频率进行划分，即\(\{j|\mathbb{E}_j>tau^j \}\) 与\(\{j|\mathbb{E}_j<=tau^j \}\)。
...
The distribution of the number of exercise knowledge points in this data set is shown in the figure. The number of questions involved in different knowledge points is different. We define the set of questions involved in knowledge points \(j\) as \(\mathbb{E}_j\). Remember the threshold of the number of occurrences of the knowledge point label \(\tau^j \), then the knowledge points of the exercises can be divided according to the frequency of occurrence, that is, \( \{j|\mathbb{E}_j|>tau^j\} \) and \( \{j||\mathbb{E}_j|<=tau^j \} \). According to the different thresholds, the ability of the model to classify knowledge points that appear frequently and that appear sparse can be tested separately.
...
\end{align}
% Coverage: In the average ranking of predicted labels for each sample, how many predicted labels need to be searched to find the true labels of all samples, where \(rank(x,y)\) represents the ranking of predicted labels, namely
% \begin{align}
% 	\operatorname{coverage}(h)=\frac{1}{c} \sum_{i=1}^{c} \max _{y \in Y_{i}} \operatorname{rank}_{f}(x_{i}, y)-1
% \end{align}
% Average Precision (Average Precision): In the predicted label ranking of all samples, the one that is ranked before the real label is also the average of the probability of the real label, that is
% \begin{align}
% 	\operatorname{AP}(h)=\frac{1}{c} \sum_{i=1}^{c} \frac{1}{|Y_{i}|} \sum_{y \in Y_{i}} \frac{|\{y^{\prime} \mid \operatorname{rank}_{f}(x_{i}, y^{\prime}) \leq \operatorname{rank}_{f}(x_{i}, y), y^{\prime} \in Y_{i}\}|}{\operatorname{rank}_{f}(x_{i}, y)}
% \end{align}
...
% 	      \begin{align}
% 		      \operatorname{SubAcc}(h)=\frac{1}{p} \sum_{i=1}^{p}[{h}(x_{i}), Y_{i}] \label{fml:metrics1}
% 	      \end{align}
...
% 	      \begin{align}
% 		      \operatorname{ Precision }(h)=\frac{1}{p} \sum_{i=1}^{p} \frac{|Y_{i} \cap h(x_{i})|}{|h(x_{i})|}
% 	      \end{align}
% 	\item Recall: The ratio of true to true in prediction is true, the calculation formula is
% 	      \begin{align}
% 		      \operatorname{Recall}(h)=\frac{1}{p} \sum_{i=1}^{P} \frac{|Y_{i} \cap h(x_{i})|}{|Y_{i}|}
% 	      \end{align}
...
%在实验中，设定了200，150，100，50，10五组阈值\(\tau_j \)，过滤出大于该频次阈值的习题作为测试集，并分别统计该习题集的平均知识点数\(L_{avg}\)。
In the experiment, five sets of thresholds of 200, 150, 100, 50, 10 are set, and the exercises larger than the frequency threshold \(\tau \)are filtered out as the test set, and the average knowledge points  \(L_{avg}\).of the exercise set are counted.

\begin{table}[h]
	\centering
	\caption{Setting of Experiment}\label{tbl:ch2-ex1}
	\begin{tabular}{cccc}
		\hline \text{\(\tau \)} & \text{ Number of Knowledge Points } & \text{ Number of Exercises } & \text{ 标签的勢 } \\
		\hline 100              & 3                                   & 498                          & 1.02              \\
		50                      & 17                                  & 940                          & 1.53              \\
		40                      & 23                                  & 1127                         & 1.51              \\
		30                      & 30                                  & 1224                         & 1.60              \\
		20                      & 35                                  & 1270                         & 1.64              \\
...
\section{Summary}
%现阶段教育活动中的知识点标注主要是凭借教师人工标注, 存在成本高、 效率低、 准确度难以保障等问题。因此, 自动标注试题知识点成为了亟待解决的问题。本章提出一个基于图神经网络和基于注意力机制Bi-LSTM文本挖掘的习题知识点标注的模型。使用实际背景下的高中数学试题进行了实验, 实验结果表明在实际背景下文本提出的方法要优于已有的基于传统机器学习的多标签文本分类方法。
