...
%在现实世界中，许多重要数据集以图或网络的形式存在，例如社交网络，知识图，蛋白质相互作用网络，世界贸易网络等。一些论文对此问题进行了回顾，并试图对神经网络进行概括，并将其应用于任意图结构数据\引用{wu2018socialgcn，dettmers2018convolutional}。当前，大多数图神经网络模型具有通用的体系结构。这些模型称为图卷积神经网络（GCN），是卷积的，因为可以在图中的所有位置或局部位置共享过滤器参数。对于这些模型，他们的目标是学习图形$ G =（V，E）$上信号或特征的映射。他们的输入包括：

% \ begin {itemize}
% \ item每个节点$ i $的特征描述$ x_i $可以写为$ N \ times D $特征矩阵（$ N $代表节点数，$ D $代表输入特征数）
% \ item以矩阵形式表示的图结构的特征描述，通常以邻接矩阵的形式表示，表示为$ A $
% \ end {itemize}

% 该模型将产生节点级输出$ Z $（$ N \倍F $特征矩阵，其中$ F $代表每个节点的输出特征数）。图级输出可以通过引入一些池化操作来建模。
...
% $ L $是层数。核心问题是如何选择和参数化$ f $。通过应用\ cite {kipf2016semi}的卷积运算，可以将$ f $表示为\ ref {fml：gcn1}：
% \ begin {align}
% H ^ {（（l + 1）}＆= \ sigma \ left（\ tilde {D} ^ {-\ frac {1} {2}} \ tilde {A} \ tilde {D} ^ {-\ frac {1 } {2}} H ^ {（l）} W ^ {{l}} \ right）\ label {fml：gcn1}
% \ end {align}
% $ l $层网络的输入为$ H ^ {{（l）} \ in \ mathbb {R} ^ {N \ times D} $（初始输入为$ H ^ {（0）} $） ，$ N $是图中的节点数，每个节点由$ D $维特征向量表示。 $ \ tilde {A} = A + I_N $是添加了自连接的邻接矩阵，$ \ tilde {D} $是度矩阵，$ \ tilde {D_ {ii}} = \ sum_ {j} {\波浪号{A} _ {ij}} $。 \ mathbb {R} ^ {D \ times D} $中的$ W ^ {（l）} \是要训练的参数。 $ \ sigma $是相应的激活函数，例如ReLU。
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.5\textwidth]{ch2-fig14.png}
	\includegraphics[width=0.4\textwidth]{ch2-fig15.png}
