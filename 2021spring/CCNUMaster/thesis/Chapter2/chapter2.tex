...
This chapter's main contribution is characterizing the relationships between knowledge points with graph networks' node information propagation mechanism, enabling the model to associate hidden related knowledge points through the tagged classified knowledge points. A multi-knowledge point label classification method for exercises is proposed, which uses the textual information described by the exercise text to identify the knowledge points related to the exercise, essentially a text classification task. The proposed method is divided into two parts: a bidirectional LSTM textual information representation network using an attention-based mechanism and a multi-knowledge point labeled classifier generation network based on a graph convolutional network propagation mechanism. In the text information representation network, both the exercise and knowledge point texts are used as input vectors, and the output exercise text is used as the final exercise information representation. In contrast, the knowledge point text vector is further learned by graph convolutional network propagation in the label generator network to embed the interrelationships among knowledge points. The final exercise representation vector is calculated by multiplying with the output knowledge point classification to obtain the exercises' knowledge point labels. After experimental validation, the proposed model outperforms the original DKVMN and other baseline models in all metrics.
...
%In this chapter, a multi-knowledge point label classification method for exercises is proposed. The method is divided into two parts: a text information characterization module using Attentional bidirectional LSTM and a multi-knowledge point labeling classifier generation module based on GCN network propagation mechanism. Both exercises and labels utilize textual information as representations，which are generated through the text representation module.
...
\begin{itemize}
    \item Segmentation: Since the raw data has a large number of phrases and nonsensical words and phrases, a word separation step is required to remove irrelevant words and phrases. In this step, the model first removes irrelevant punctuation and stop words, and then removes words from the user dictionary. The user dictionary contains mainly redundant words from the text of mathematical exercises.
...
In this model, each node is a representation of a knowledge point, where it can be characterized by a vector of knowledge point text representations. The graph propagation learning of related knowledge points can produce the fusion of related knowledge point representations, which in turn improves the associative learning performance of model recognition for related knowledge points.
...
This section use web crawlers to obtain the data, so the original data obtained contains a large number of unstructured exercises with irregular mathematical symbols, which will bring errors so that the unrecognizable characters will be replaced in the preprocessing process. There is also a large amount of nonsensical text. Besides, since some of the exercises are not fully labeled with knowledge points, manual assistance is required to label the knowledge points of training dataset.


\subsubsection{Data Analysis}
Since there are many images in the original set of exercises that cannot be identified, this question screens out irrelevant image URL information. Also, this model does not consider textual information such as formulas and mathematical symbols, so only Chinese text will be screened out. Samples with too low text length will be removed.
...
This section's study focuses on a text multi-label classification task, so several more popular text classification models are used as baseline models. To verify the proposed model's advancements, some currently popular text classification models are selected experimentally, and the performance is compared with the proposed model on the selected experimental evaluation metrics and the same dataset. At present, there are already some algorithm models for labeling text. To verify the effectiveness of the algorithm proposed in this thesis, the following baseline models will be set as comparisons.
\begin{itemize}
...
\begin{table}[htbp!]
    \caption{Result comparison}\label{tbl:ch2-result-bsline1}
    \centering
    \begin{tabular}{ccccc}
        \toprule
        ...
        Proposed         & \textbf{0.912}                & \textbf{0.932}                & \textbf{0.888}              & \textbf{0.937}             \\
        \bottomrule
    \end{tabular}
\end{table}
...
In the exercise recommendation system, there are a large number of exercises with missing knowledge point labels. It is necessary to label the exercises to meet the adaptive learning system's requirements, while the manual labeling is high cost and low efficiency. Therefore, automatic marking of knowledge points of test questions is an critical problem to be solved. This chapter proposes a multi-knowledge point labeling model for exercises based on GCN and Bi-LSTM text mining model based on the attention mechanism. For proposed model, a better knowledge point labeling performance than existing models has been achieved after verifying the experimental data set.

The contributions of this chapter are as follows:
\begin{enumerate}
    \item The relationship between knowledge points is modeled by a graph neural network, which can dig out the hidden knowledge point labels in the original text and provide the existing model with the latent knowledge point reasoning capability.
    \item By designing the correlation function between the knowledge points, the dependence between the knowledge points is modeled, which was based on the co-existence of knowledge points.
    \item Verified that little size of training sample will produce an over-fitting for the multi-label classification model, resulting in performance degradation.
\end{enumerate}

The labeling of exercise knowledge points is the first step of the recommendation system. The labeled exercises can be used as the input of the knowledge tracing model to track students' knowledge state and can also be used as the recommendation system's input feature to complete the recommendation of exercises based on knowledge points.
