...
In this model, each node is a representation of a knowledge point, where it can be characterized by a vector of knowledge point text representations. The graph propagation learning of related knowledge points can produce fusion of related knowledge point representations, which in turn improves the associative learning performance of model recognition for related knowledge points.
...
The study in this section is a text multi-label classification task, so several of the more popular text classification models are used as baseline models. At present, there are already some algorithm models for labeling text. To verify the effectiveness of the algorithm proposed in this thesis, the following baseline models will be set as comparisons.
\begin{itemize}
    \item Bi-LSTM+Attention~\cite{zhou2016attention}:
    \item Multi-label BERT\@: Use BERT to convert the test question text into an embedded vector, extract the test text information through CNN, and then output the multi-label prediction.
\end{itemize}
...
In this experiment, the training and test sets are divided in the ratio of \(3:7\), and one tenth of the training set is taken as the validation set. The running environment are shown in \tblname{{\ref{tbl:ch2-exp-env}}}. Furthermore, there are many adjustable hyperparameters within the model, several critical ones of which are shown in \tblname{\ref{tbl:ch2-hpsetting}}.
