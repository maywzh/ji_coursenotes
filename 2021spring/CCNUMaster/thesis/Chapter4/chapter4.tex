...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \STATE \algostage{1}{Compute the users' similarity.}
        ...
        \FOR{\(i \in [1,N]\)}
        \IF{\(\mathcal{S}_i\geq\tau^{simuser}\) and \(i\neq u\)}
        \STATE \(\mathbf{U}_{sim} \leftarrow \mathbf{U}_{sim}\cup \{i\} \)
        \ENDIF
        \ENDFOR
        \STATE \algostage{3}{Aggregate exercise set of similar users.}
        \STATE Initialize the similar exercise set \(\mathbf{E}^{(UCF)}\);
        \FOR{\(i \in \mathbf{U}_{sim}\)}
...
\begin{algorithm}[htbp!]
    ...
    \begin{algorithmic}[1]
        ...
        \ENSURE~The final ranked recommendation exercise list \(\mathbf{E}^{(RANK)} \). %算法的输出：Output
        \STATE \algostage{1}{Predicting correctness of exercise.}
        \STATE Initialize the predicting correctness probability vector \(\mathbf{p}=\{p_1,p_2,\ldots,p_n\}\);
        \FOR{\(e_i\ \in \mathbf{E}^{(Candidate)}\)}
        \STATE \(p_i \leftarrow F(e_i)\)
        \ENDFOR
        \STATE \algostage{2}{Sort the exercise by correctness probability.}
        \STATE Initialize the ranked exercise list \(\mathbf{L}\);
        \STATE \(\mathbf{L} \leftarrow Sort(\mathbf{E}^{(Candidate)}, \mathbf{p})\)
        \STATE \algostage{3}{Generate the final ranked exercise set.}
