...
%本实验采用KFold方法分割测试集和验证集，取\(K=5\)。原始数据经过处理，会将每个学生的做题序列进行隔离，超过预定最长序列长度的学生会被分割成若干个不超过最长长度的序列。实验运行环境为Ubuntu20.04，TensorFlow 2.23，Python 3.8.6，实验机器为一块Tesla V100计算卡。
For each dataset, 70\% is used as the training set, and the remaining 30\% is taken as the validation set. The experimental running environment is Ubuntu20.04, Python 3.8.6, Pytorch 1.8,  and the running machine is equipped with a Tesla V100 computing card.
...
\subsection{Hyperparameter Adjusting and Model Training}
