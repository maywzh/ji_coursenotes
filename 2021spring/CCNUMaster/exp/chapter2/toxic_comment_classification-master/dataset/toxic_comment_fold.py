...
class ToxicComment(object):
    ...
    def load_data(self):
        if os.path.exists(self.dir_save):
            ...
            list_sentences_train = train_data["comment_text"].fillna(
                self.NAN_WORD).values
            list_sentences_test = test_data["comment_text"].fillna(
                self.NAN_WORD).values
    ...
    def get_fold_by_id(self, idx):
        ...
        x_t = np.concatenate(
            [self.x_train[:fold_start], self.x_train[fold_end:]])
        y_t = np.concatenate(
            [self.y_train[:fold_start], self.y_train[fold_end:]])
