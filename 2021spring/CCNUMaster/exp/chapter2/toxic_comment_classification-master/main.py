...
from dataset.toxic_comment import ExerciseDataset
...
def get_parser():
    ...
    parser.add_argument('--sen_len',        default=None,
                        type=int, help='sentence length')

    parser.add_argument('--net_name',       default=None, help='[lstm]')
    parser.add_argument('--dir_date',       default=None,
                        help='Name it with date, such as 20180102')
    parser.add_argument('--batch_size',     default=32,
                        type=int, help='Batch size')
    parser.add_argument('--lr_base',        default=1e-3,
                        type=float, help='Base learning rate')
    parser.add_argument('--lr_decay_rate',  default=0.1,
                        type=float, help='Decay rate of lr')
    parser.add_argument('--epoch_lr_decay', default=1000,
                        type=int, help='Every # epoch, lr decay lr_decay_rate')

    parser.add_argument('--layer_num',      default=2,
                        type=int, help='Lstm layer number')
    parser.add_argument('--hidden_size',    default=64,
                        type=int, help='Lstm hidden units')
    parser.add_argument('--gpu',            default='0', help='GPU id list')
    parser.add_argument('--workers',        default=4,
                        type=int, help='Workers number')
...
def main(args):
    ...
    if args.phase == 'Train':
        model = Net(in_features=300, hidden_size=args.hidden_size,
                    layer_num=args.layer_num, phase='Train')
        ...
        data_set = ExerciseDataset(
            dir_data=dir_data, sentence_length=args.sen_len, phase='Train')
        loader = DataLoader(data_set, batch_size=args.batch_size,
        ...
        while True:
            adjust_learning_rate(
                optimizer, epoch_current, args.lr_base, args.lr_decay_rate, args.epoch_lr_decay)
            loss_list = []
    ...
    model = Net(in_features=300, hidden_size=args.hidden_size,
                layer_num=args.layer_num, phase='Test')
    if torch.cuda.is_available():
        model = nn.DataParallel(model.cuda(), device_ids=args.gpu)
    ...
    data_set = ExerciseDataset(
        dir_data=dir_data, sentence_length=args.sen_len, phase='Test')
    loader = DataLoader(data_set, batch_size=args.batch_size,
                        shuffle=False, num_workers=args.workers)
    predicts = []  # 153164
    for step, sample_batch in enumerate(tqdm(loader)):
        predict = run_batch(sample=sample_batch,
                            model=model, phase='Test')  # bs x 6
...
if __name__ == '__main__':
    ...
    # set visible gpu list, eg: '2,3,4'
    os.environ["CUDA_VISIBLE_DEVICES"] = parser_args.gpu
    # store the gpu id into a list
    gpu_list = re.split('[, ]', parser_args.gpu)
    parser_args.gpu = range(
        len(list(filter(None, gpu_list))))  # gpu for PyTorch
