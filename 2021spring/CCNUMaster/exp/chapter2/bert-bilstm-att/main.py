...
dataset = data.TabularDataset(path='all.csv', format='csv', fields=[
                              ('text', TEXT), ('label', LABEL)], skip_header=True)
train_dt, valid_dt, test_dt = dataset.split(
...
train_it, valid_it, test_it = data.BucketIterator.splits((train_dt, valid_dt, test_dt), batch_sizes=(
    bsize, bsize, bsize), device=device, sort_key=lambda x: len(x.text), repeat=False)
