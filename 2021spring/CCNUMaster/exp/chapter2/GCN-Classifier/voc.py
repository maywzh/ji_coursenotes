...
from PIL import exercise
import pickle
import util
from util import *

object_categories = ['linear algebra', 'bicycle', 'bird', 'boat',
...
urls = {
    ...
    'test_exercises_2007': 'http://host.robots.ox.ac.uk/pascal/VOC/voc2007/VOCtest_06-Nov-2007.tar',
    'test_anno_2007': 'http://host.robots.ox.ac.uk/pascal/VOC/voc2007/VOCtestnoimgs_06-Nov-2007.tar',
}


def read_exercise_label(file):
...
def read_object_labels(root, dataset, set):
    path_labels = os.path.join(
        root, 'VOCdevkit', dataset, 'exerciseSets', 'Main')
    labeled_data = dict()
...
def find_exercises_classification(root, dataset, set):
    path_labels = os.path.join(
        root, 'VOCdevkit', dataset, 'exerciseSets', 'Main')
    exercises = []
...
def download_voc2007(root):
    ...
    path_exercises = os.path.join(
        root, 'VOCdevkit', 'VOC2007', 'JPEGexercises')
    ...
    # train/val exercises/annotations
    if not os.path.exists(path_exercises):

        # download train/val exercises/annotations
        ...
        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(
                urls['trainval_2007'], cached_file))
    ...
    test_anno = os.path.join(
        path_devkit, 'VOC2007/exerciseSets/Main/aeroplane_test.txt')
    if not os.path.exists(test_anno):

        # download test annotations
        parts = urlparse(urls['test_exercises_2007'])
        filename = os.path.basename(parts.path)
        cached_file = os.path.join(tmpdir, filename)

        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(
                urls['test_exercises_2007'], cached_file))
            util.download_url(urls['test_exercises_2007'], cached_file)
    ...
    # test exercises
    test_exercise = os.path.join(
        path_devkit, 'VOC2007/JPEGexercises/000001.jpg')
    if not os.path.exists(test_exercise):

        # download test exercises
        ...
        if not os.path.exists(cached_file):
            print('Downloading: "{}" to {}\n'.format(
                urls['test_anno_2007'], cached_file))
...
class Voc2007Classification(data.Dataset):
    def __init__(self, root, set, transform=None, target_transform=None, inp_name=None, adj=None):
        ...
        self.path_exercises = os.path.join(
            root, 'VOCdevkit', 'VOC2007', 'JPEGexercises')
        self.set = set
        ...
        self.exercises = read_object_labels_csv(file_csv)

        with open(inp_name, 'rb') as f:
            self.inp = pickle.load(f)
        self.inp_name = inp_name

        print('[dataset] VOC 2007 classification set=%s number of classes=%d  number of exercises=%d' % (
            set, len(self.classes), len(self.exercises)))

    def __getitem__(self, index):
        path, target = self.exercises[index]
        img = exercise.open(os.path.join(
            self.path_exercises, path + '.jpg')).convert('RGB')
