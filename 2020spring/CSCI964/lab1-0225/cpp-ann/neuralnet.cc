...
NeuralNet::NeuralNet(int input_size, int output_size,  std::vector<Matrix> weights, Matrix inputs):
input_size{input_size}, output_size{output_size}, weights{weights}, inputs{inputs} {
	int prev_dim = inputs.getCols();
	for (unsigned int i = 0; i < hidden_sizes.size(); ++i) {
		Matrix weight_layer {prev_dim, hidden_sizes[i]};
		prev_dim = hidden_sizes[i];
