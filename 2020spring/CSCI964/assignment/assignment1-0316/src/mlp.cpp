...
void TrainNet2(float **x, float **d, int NumIPs, int NumOPs, int NumPats,
  ...
  float PatErr, MinErr, AveErr = 0.0, MaxErr, lastAveErr,
                        PcntErr = 0.0; // Pattern errors
  int p, i, j;                         // for loops indexes
  long ItCnt = 0;                      // Iteration counter
  long NumErr = 0; // Error counter (added for spiral problem)

  cout << "TrainNet2: IP:" << NumIPs << " H1:" << NumHN1 << " OP:" << NumOPs
       << endl;
  ...
  for (;;) {
    ...
    MaxErr = -3.4e38;
    NumErr = 0;
    // bool WrongClassified = NumErr > 1 / 10 * NumPats; // if 1/10 samples are
    // recorded, the epoch/pattern is wrong classified
    if (Ordering == 0) {
    } else if (Ordering == 1) {
      RandomArray(arr, NumPats, 1);
    } else if (Ordering == 2) {
    ...
    for (int idx = 0; idx < NumPats; idx++) {
      ...
      for (i = 0; i < NumOPs; i++) {
        ...
        NumErr += ((y[i] < 0.5 && d[p][i] >= 0.5) ||
                   (y[i] >= 0.5 &&
                    d[p][i] < 0.5)); // added for binary classification problem
      }
      if (PatErr < MinErr)
        MinErr = PatErr;
      if (PatErr > MaxErr)
      ...
      for (i = 0; i < NumOPs; i++) { // Modify layer 2 wts
        ...
        for (j = 0; j < NumHN1; j++) {
          w2[j][i] += LrnRate * h1[j] * ad2[i] + Mtm1 * (w2[j][i] - w22[j][i]) +
    ...
    cout << setprecision(6) << setw(6) << ItCnt << ": " << setw(12) << MinErr
         << setw(12) << AveErr << setw(12) << MaxErr << setw(12) << PcntErr
         << endl;
    trainfile.setf(ios::fixed | ios::showpoint);
    trainfile << MinErr << setw(12) << AveErr << setw(12) << MaxErr << setw(12)
              << PcntErr << endl;
    if (((abs(AveErr - lastAveErr) <= 0.000001) && (AveErr < ObjErr)) ||
        (ItCnt == NumIts)) {
...
void TrainNet3(float **x, float **d, int NumIPs, int NumOPs, int NumPats,
  ...
  cout << "TrainNet2: IP:" << NumIPs << " H1:" << NumHN1 << " H2:" << NumHN2
       << " OP:" << NumOPs << endl;
  ...
  for (;;) {
    ...
    MaxErr = -3.4e38;
    NumErr = 0;
    bool WrongClassified =
        NumErr > 1 / 10 * NumPats; // if 1/10 samples are recorded, the
                                   // epoch/pattern is wrong classified
    if (Ordering == 0) {
    } else if (Ordering == 1) {
      RandomArray(arr, NumPats, 1);
    } else if (Ordering == 2) {
    ...
    for (int idx = 0; idx < NumPats; idx++) {
      ...
      for (i = 0; i < NumOPs; i++) {
        ...
        for (j = 0; j < NumHN2; j++) {
          w3[j][i] += LrnRate * h2[j] * ad3[i] + Mtm1 * (w3[j][i] - w33[j][i]) +
      ...
      for (i = 0; i < NumHN2; i++) {
        ...
        for (j = 0; j < NumHN1; j++) {
          w2[j][i] += LrnRate * ad2[i] * h1[j] + Mtm1 * (w2[j][i] - w22[j][i]) +
                      Mtm2 * (w22[j][i] - w222[j][i]);
          w222[j][i] = w22[j][i];
          w22[j][i] = w2[j][i];
        }
...
void TrainNet4(float **x, float **d, int NumIPs, int NumOPs, int NumPats,
  ...
  for (;;) {
    ...
    NumErr = 0;
    bool WrongClassified =
        NumErr > 1 / 10 * NumPats; // if 1/10 samples are marked error, the
                                   // epoch/pattern is wrong classified
    if (Ordering == 0) {
    } else if (Ordering == 1) {
      RandomArray(arr, NumPats, 1);
...
void TestNet2(float **x, float **d, int NumIPs, int NumOPs, int NumPats) {
  ...
  float PcntErr = NumErr / float(NumPats) * 100.0;
  cout.setf(ios::fixed | ios::showpoint);
  cout << "MinErr:" << setw(12) << MinErr << " AveErr:" << setw(12) << AveErr
       << " MaxErr:" << setw(12) << MaxErr << " PcntErr:" << setw(12) << PcntErr
       << endl;
  ofstream testfile;
  testfile.open("./testresult.txt");
  testfile << MinErr << setw(12) << AveErr << setw(12) << MaxErr << setw(12)
           << PcntErr << endl;
  testfile.close();
  delete[] h1;
  delete[] y;
...
int main() {
  ...
  if (NumIPs < 1 || NumIPs > MAXN || NumOPs < 1 || NumOPs > MAXN ||
      ...
      NumTrnPats > MAXPATS || NumIts < 1 || NumIts > 20e6 || NumHN1 < 0 ||
      NumHN1 > 50 || LrnRate < 0 || LrnRate > 1 || Mtm1 < 0 || Mtm1 > 10 ||
      Mtm2 < 0 || Mtm2 > 10 || ObjErr < 0 || ObjErr > 10) {
  ...
  // 按行读取训练的数据 前 NumIPs 个为输入神经元(feature) 后 NumOPs
  // 个为输出神经元(target)
  ...
  // 按行读取测试的数据 前 NumIPs 个为输入神经元(feature) 后 NumOPs
  // 个为输出神经元(target)
  ...
  if (NumHN == 1)
    ...
    paramfile << " HL1=" << NumHN1 << " HL2=" << NumHN2 << " HL3=" << NumHN3
              << endl;
  paramfile.close();
