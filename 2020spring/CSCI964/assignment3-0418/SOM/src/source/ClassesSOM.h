  ...
  void ReadData(const char *filename, int NumberVectors, int InputVectorSize) {
    ...
    while (std::getline(data, line)) {
      ...
      while (std::getline(lineStream, cell, ',')) {
        ...
        if (std::stof(cell) > GetInputVectors::MaxValueInputData) {
          GetInputVectors::MaxValueInputData = std::stof(cell);
        }
        colIndex++;
      }
...
public:
  ...
  float MaxValueInputData; // the maximum value in the input data. To have the
                           // same range of values in the SOM
  int xsize, ysize;
  ...
  void InitializeMap(int xsize, int ysize) {
    ...
    for (int i = 0; i < xsize; i++) {
      for (int j = 0; j < ysize; j++) {
        ...
        now =
            (now.cwiseAbs() / now.cwiseAbs().maxCoeff()) *
            GetInputVectors::MaxValueInputData; // random values between 0 and 1
        SOMMap(i, j) = now;
      }
    }
...
//      BMU(t+1) = BMU(t) + LearningRate(t) *
//      NeighbourhoodFunction(t)*[InputVector - BMU(t)]
// NeighbourhoodFunction
//
class SOMNeighbourhoodFunction {
public:
  float NeighbourhoodFunction(int BMUX, int BMUY, int BMUNowX, int BMUNowY,
                              float SigmaNeighbouring) {
    float value = (float)(pow(BMUX - BMUNowX, 2) + pow(BMUY - BMUNowY, 2)) /
                  (float)(2 * pow(SigmaNeighbouring, 2));
    value = exp(-value);
    // value = exp(value);
    // std::cout  << (float)(pow(BMUX - BMUNowX,2) + pow(BMUY - BMUNowY,2)) <<
    // (float)2*pow(SigmaNeighbouring,2) << value << "\n";
    // std::cout << value << "\n";
    return value;
  }
};
...
public:
  Eigen::VectorXd LearnFunction(Eigen::VectorXd BMUNow, Eigen::VectorXd Input,
                                int BMUX, int BMUY, int BMUNowX, int BMUNowY,
                                float SigmaNeighbouring, float LearningRate) {
    Eigen::VectorXd value =
        BMUNow + LearningRate *
                     NeighbourhoodFunction(BMUX, BMUY, BMUNowX, BMUNowY,
                                           SigmaNeighbouring) *
                     (Input - BMUNow);
    return value;
  }
...
public:
  float UpdateLearningRate(float L0, float LearningRateFinal, float Iter,
                           float lambda) {
    float value =
        (L0 - LearningRateFinal) * L0 * exp(-((float)Iter / (float)lambda)) +
        LearningRateFinal;
    return value;
  }
};
...
public:
  float UpdateSigmaNeighbouring(float SigmaNeighbouringZero,
                                float SigmaNeighbourhoodFinal, float Iter,
                                float lambda) {
    float value = (SigmaNeighbouringZero - SigmaNeighbourhoodFinal) *
                      exp(-((float)Iter / (float)lambda)) +
                  SigmaNeighbourhoodFinal;
    return value;
  }
};
////////////////////////////////////////////////////////////////////////////////////////////
...
class PrintSOM {
public:
  void PrintToCSVFileRowWise(const char *filename, MatrixOfVectors SOMMap,
                             int xsize, int ysize, int InputVectorSize) {

    std::ofstream file(filename);
    if (file.is_open()) {
      for (int i = 0; i < xsize;
           i++) { // xsize is considered the rows of the SOM (i or RowMap
                  // below), and ysize the columns (j or ColMap below)
        for (int j = 0; j < ysize; j++) {
...
class SOMTrain : public Initialize2DMap,
                 public GaussianDistribution,
                 public SOMLearningFunction,
                 public PrintSOM,
                 public EuclideanDistance,
                 public UpdateSOMLearningRate,
                 public UpdateSOMSigmaNeighbouring,
                 public RandomGenerator {
public:
  int Iters;
  int BestXMap, BestYMap = 0;
  float Score, ScoreNow = 0;
  float LearningRateInitial, LearningRateFinal, SigmaNeighbouringInitial,
      SigmaNeighbourhoodFinal, SigmaNeighbouring, LearningRate, lambda;

  void Train(int Iters) { //, MatrixOfVectors InputVectors, int xsize, int
                          // ysize, MatrixOfVectors SOMMap
    // Initialize alpha
    ...
    for (int i = 0; i < Iters; i++) {
      // for (int j=0; j<SOMTrain::InputVectors.rows();j++){ //XX the order
      // should be random
      ...
      for (int RowMap = 0; RowMap < SOMTrain::xsize; RowMap++) {
        for (int ColMap = 0; ColMap < ysize; ColMap++) {
          ScoreNow = EvaluateDistance(Initialize2DMap::SOMMap(RowMap, ColMap),
                                      InputVectors(j, 0)); // XX ,1.0 sigma
          if (ScoreNow > SOMTrain::Score) {
            SOMTrain::Score = ScoreNow;
            SOMTrain::BestXMap = RowMap, SOMTrain::BestYMap = ColMap;
          }
        }
      }
      ...
      for (int RowMap = 0; RowMap < SOMTrain::xsize; RowMap++) {
        for (int ColMap = 0; ColMap < ysize; ColMap++) {
          ...
          if (CoordX >= (SOMTrain::xsize / 2)) {
            CoordX = (SOMTrain::xsize / 2) - RowMap % (SOMTrain::xsize / 2);
          }
          // if (CoordY < (SOMTrain::ysize/2)) {CoordY = ColMap;}
          if (CoordY >= (SOMTrain::ysize / 2)) {
            CoordY = (SOMTrain::ysize / 2) - ColMap % (SOMTrain::xsize / 2);
          }
          // std::cout << CoordX << "\n";// ;<< "  " << CoordY << std::endl;

          Eigen::VectorXd BMUUpdated = LearnFunction(
              Initialize2DMap::SOMMap(RowMap, ColMap), InputVectors(j, 0),
              SOMTrain::BestXMap, SOMTrain::BestYMap, CoordX, CoordY,
              SOMTrain::SigmaNeighbouring, SOMTrain::LearningRate);
      ...
      SOMTrain::LearningRate =
          UpdateLearningRate(SOMTrain::LearningRateInitial,
                             SOMTrain::LearningRateFinal, i, SOMTrain::lambda);
      SOMTrain::SigmaNeighbouring = UpdateSigmaNeighbouring(
          SOMTrain::SigmaNeighbouringInitial, SOMTrain::SigmaNeighbourhoodFinal,
          i, SOMTrain::lambda);
    }
  }
};
